...
ArrayList* al_subList(ArrayList* pList,int from,int to)
{
    ...
    void* auxGet;
    int i;
    if(pList!=NULL && from >=0 && to>=0 && from < to && to <= pList->size)
    {
        returnAux = al_newArrayList();
        for(i=from; i<to; i++)
        {
        ...
        if(order==1)
        {
            for(i=0;i<pList->size-1;i++)
            {
                for(j=i+1;j<pList->size;j++)
                {
                    ...
                    if(valor == 1)
                    {
                        auxCompare = *(pList->pElements+i);
                        *(pList->pElements+i) = *(pList->pElements+j);
                        *(pList->pElements+j) = auxCompare;
                    }
                }
            }
            returnAux=0;
        ...
        if(order==0)
        {
            for(i=0;i<pList->size-1;i++)
            {
                for(j=i+1;j<pList->size;j++)
                {
                    ...
                    if(valor == -1)
                    {
                        auxCompare = *(pList->pElements+j);
                        *(pList->pElements+j) = *(pList->pElements+i);
                        *(pList->pElements+i) = auxCompare;
                    }
                }
            }
            returnAux=0;
